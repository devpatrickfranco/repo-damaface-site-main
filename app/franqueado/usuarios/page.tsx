...
export default function UsuariosPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Em franquias, o formulário é dividido em etapas: enquanto não chegar
    // na última, o "submit" (botão Próximo ou Enter) apenas avança a etapa.
    ...
    try {
      if (modalType === "usuario") {
        ...
        const franquiaData = new FormData()
        franquiaData.append("nome", formData.nome)
        franquiaData.append("cnpj", formData.cnpj)
        franquiaData.append("cidade", formData.cidade)
        franquiaData.append("estado", formData.estado)
        franquiaData.append("bairro", formData.bairro)
        franquiaData.append("endereco", formData.endereco)
        franquiaData.append("numero", formData.numero)
        franquiaData.append("cep", formData.cep)
        franquiaData.append("telefone", formData.telefone)
        franquiaData.append("whatsapp", formData.whatsapp)
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-in fade-in duration-200">
          <div className="bg-gray-800 rounded-xl border border-gray-600 shadow-2xl w-full max-w-md md:max-w-2xl lg:max-w-3xl max-h-[90vh] flex flex-col overflow-hidden transform animate-in slide-in-from-bottom-4 duration-300">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0">
              {/* Indicador de etapas (somente franquia) */}
              {modalType === "franquia" && (
                ...
                {modalType === "usuario" ? (
                  ...
                  <>
                    ...
                    {modalStep < franquiaSteps.length ? (
                      <button
                        type="submit"
                        className="px-6 py-3 bg-gradient-to-r from-pink-500 to-pink-600 hover:from-pink-600 hover:to-pink-700 text-white rounded-lg font-medium flex items-center justify-center space-x-2 transition-all duration-200 shadow-lg hover:shadow-pink-500/25"
                      >
                        <span>Próximo</span>
