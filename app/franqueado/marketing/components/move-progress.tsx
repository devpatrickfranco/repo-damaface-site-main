...
import { X, MoveRight, CheckCircle2, AlertCircle, Loader2, FileIcon, ChevronDown, ChevronUp } from "lucide-react"
import { useState } from "react"
import type { MoveProgress } from "@/types/marketing"
...
export function MoveProgressToast({ progress, onCancel }: MoveProgressProps) {
                ...
                <div className="flex items-center gap-3 flex-1">
                    ...
                    <div className="flex-1 min-w-0">
                        <h3 className="text-sm font-semibold text-white">
                            {isComplete
                                ? hasErrors
                                    ? `${progress.completed - progress.failed} item${progress.completed - progress.failed !== 1 ? 'ns' : ''} movido${progress.completed - progress.failed !== 1 ? 's' : ''}`
                                    : `${progress.total} item${progress.total !== 1 ? 'ns' : ''} movido${progress.total !== 1 ? 's' : ''}`
                                : `Movendo ${progress.total} ${progress.total === 1 ? 'item' : 'itens'}`}
                        </h3>
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <div className="flex items-center gap-2 mt-1">
                                        ...
                                        <span className="text-xs text-purple-400">Movendo...</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div className="flex-1">
                                    <p className="text-sm text-white font-medium">
                                        {progress.completed} item{progress.completed !== 1 ? 'ns' : ''} movido{progress.completed !== 1 ? 's' : ''}
                                    </p>
                                    {hasErrors && (
...
export function MoveProgressInline({ progress }: { progress: MoveProgress | null }) {
    if (!progress) return null
