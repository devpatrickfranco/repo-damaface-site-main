// app/dashboard/blog/edit/[id]/EditPostForm.tsx
'use client';
...
function SubmitButton() {
  ...
  return (
    <button type="submit" disabled={pending} className="bg-brand-pink text-white px-4 py-2 rounded-lg hover:opacity-90 disabled:bg-gray-500">
...
export default function EditPostForm({ post }: { post: PostWithRelations }) {
  ...
  const [content, setContent] = useState(post.content); // Inicia com o conteúdo existente
  ...
  return (
    <div>
      ...
      <form action={dispatch} className="space-y-6 max-w-2xl">
        <div>
          ...
          <input type="text" id="title" name="title" required defaultValue={post.title} className="mt-1 block w-full bg-gray-700 border-gray-600 rounded-md shadow-sm text-white focus:ring-brand-pink focus:border-brand-pink"/>
        ...
        <div>
          ...
          <input type="text" id="slug" name="slug" required defaultValue={post.slug} className="mt-1 block w-full bg-gray-700 border-gray-600 rounded-md shadow-sm text-white focus:ring-brand-pink focus:border-brand-pink"/>
    ...
    <CategoryTagSelector selectedCategories={post.categories?.map(c => c.id) || []} selectedTags={post.tags?.map(t => t.id) || []} />
        ...
        <div className="flex items-center">
          <input id="published" name="published" type="checkbox" defaultChecked={post.published} className="h-4 w-4 rounded border-gray-600 bg-gray-700 text-brand-pink focus:ring-brand-pink"/>
