...
export function WhatsAppModule() {
  ...
  useEffect(() => {
    // Elite: Garante que temos um CID para rastrear a inicialização
  ...
  const isDegraded = status === 'failed' && isSyncing;
          ...
          {status === 'active' ? (
            <div className="space-y-6 animate-in slide-in-from-right-4 duration-500">
              <div className="p-6 bg-white border border-gray-100 rounded-3xl shadow-sm">
                <h3 className="text-lg font-black text-gray-900 mb-4">Painel de Atendimento</h3>
                <MessageInput />
              </div>
              
              <div className="grid grid-cols-2 gap-4">
                <div className="p-6 bg-[#25D366]/5 border border-[#25D366]/20 rounded-2xl relative overflow-hidden group">
                   <div className="absolute -right-4 -bottom-4 w-20 h-20 bg-[#25D366]/10 rounded-full group-hover:scale-150 transition-transform duration-700" />
                  <p className="text-3xl font-black text-[#128C7E]">0</p>
                  <p className="text-[10px] font-bold text-[#128C7E] uppercase tracking-widest flex items-center space-x-1">
                ...
                <div className="p-6 bg-blue-50 border border-blue-100 rounded-2xl">
                  ...
                  <p className="text-[10px] font-bold text-blue-600 uppercase tracking-widest">Conversas Iniciadas</p>
                </div>
              </div>
            </div>
          ) : (
            <div className="h-full min-h-[400px] flex flex-col items-center justify-center bg-gray-50/50 border border-dashed border-gray-200 rounded-3xl p-8 text-center transition-all" style={{ opacity: isSyncing ? 0.6 : 1, transform: isSyncing ? 'scale(0.98)' : 'scale(1)' }}>
